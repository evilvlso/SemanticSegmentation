...
def rescale_src(mask_src, img_src, h, w):
    ...
    rescale_ratio = np.random.uniform(0.2, 0.9)
...
if __name__ == '__main__':
    ...
    train_imgs=os.path.join(root_dir,"segTrain","imgs")
    train_mask=os.path.join(root_dir,"segTrain","mask")

    save_train_imgs=os.path.join(root_dir,"cpTrain","imgs")
    save_train_masks=os.path.join(root_dir,"cpTrain","mask")
    save_train_viz=os.path.join(root_dir,"cpTrain","viz")
    os.makedirs(save_train_imgs,exist_ok=True)
    os.makedirs(save_train_masks,exist_ok=True)
    os.makedirs(save_train_viz,exist_ok=True)
    alldata=os.listdir(train_imgs)
    all_img_src=random.sample(alldata,len(alldata)//2)
    remain=list(set(alldata).difference(set(all_img_src)))
    all_img_main=random.sample(remain,len(alldata)//2)
    for src_file,main_file in zip(all_img_src,all_img_main):
        mask_src = cv2.imread(os.path.join(train_mask,src_file.replace("jpg","png")),cv2.IMREAD_GRAYSCALE)
        img_src = imageio.imread(os.path.join(train_imgs,src_file))
        mask_main = cv2.imread(os.path.join(train_mask,main_file.replace("jpg","png")), cv2.IMREAD_GRAYSCALE)
        img_main = imageio.imread(os.path.join(train_imgs,main_file))
        # Copy-Paste data augmentation
        mask, img = copy_paste(mask_src, img_src, mask_main, img_main)
        imageio.imwrite(os.path.join(save_train_imgs,f"{src_file.replace('.jpg','')}_{main_file.replace('.jpg','')}.jpg"), img)
        imageio.imwrite(os.path.join(save_train_masks,f"{src_file.replace('.jpg','')}_{main_file.replace('.jpg','')}.png"), mask)
        save_colored_mask(mask,os.path.join(save_train_viz,f"{src_file.replace('.jpg','')}_{main_file.replace('.jpg','')}.png"))
        print(f"{src_file}->{main_file} ok")